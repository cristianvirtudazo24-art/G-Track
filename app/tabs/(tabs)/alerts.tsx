  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const mappedAlerts = broadcastData.map((item: any) => {
        const rawMessage = item.text || item.message || 'No message content';
        const { subject, body } = parseBroadcastMessage(rawMessage);
        return {
          id: String(item.id),
          title: item.title || subject || ALERT_CONFIG[item.type]?.defaultTitle || 'Admin Broadcast',
          body,
